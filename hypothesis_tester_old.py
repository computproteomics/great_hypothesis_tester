...
class HypothesisTester():
    ...
    def predict_retention_time(self, model_type: str = 'prosit'):
        ...
        if model_type == 'prosit':
            ...
            # write input to temporary .csv file
            fields = ['seq', 'modifications']

            rows = [[i,''] for i in self.sequences.values()]
            ...
            # load predictions
            os.system(f'deeplc --file_pred {filename}')

            # results
            prediction = np.array(pd.read_csv(f'query_deeplc_predictions.csv').iloc[:,1].values)

            # remove temporary files
            os.remove(filename)
            os.remove(f'query_deeplc_predictions.csv')
    ...
    def predict_intensities(self, model_type: str = 'prosit'):
        ...
        if model_type == 'prosit':
            ...
            # write inputs to .peprec file
            peprec_file_fields = ['spec_id', 'modifications', 'peptide', 'charge']

            peprec_file_rows = [[sequence_name, '-', sequence, 2] for sequence_name, sequence in self.sequences.items()]

            peprec_filename = 'intensity_query.peprec'

            with open(peprec_filename, 'w') as peprec_file:
                csvwriter = csv.writer(peprec_file, delimiter=' ')
                csvwriter.writerow(peprec_file_fields)
                csvwriter.writerows(peprec_file_rows)

            os.system('ms2pip -c ms2pip_config.txt intensity_query.peprec')

            prediction = pd.read_csv('intensity_query_HCD_predictions.csv')

            os.remove('intensity_query.peprec')
            os.remove('intensity_query_HCD_predictions.csv')

            self.intensity_stats = prediction
            return
